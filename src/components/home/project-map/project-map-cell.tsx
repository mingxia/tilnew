...
export function ProjectMapCell({ project, index, style }: { project: Project; index: number; style: CSSProperties }) {
  const Icon = icons[project.icon];
  const href = project.href ?? `/projects/${project.slug}`;
  return <a href={href} role="listitem" className={`project-map-cell status-${project.status}`} aria-label={`${project.title}，${statusLabels[project.status]}，进入项目详情`} style={{ ...style, animationDelay: `${index * 35}ms` }}>
